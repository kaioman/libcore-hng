...
### 6.3 環境別暗号化設定ファイルの選択ルール

- 暗号化設定ファイルは、環境別に `*.{APP_ENV}.*.enc` の命名を使用する
- `APP_ENV` が設定されている場合、該当環境の `.enc` のみを復号対象とする
- 環境別 `.enc` が存在する場合、汎用の `.enc` はフォールバックとしてのみ利用する
- 非対象の環境の `.enc` は読み込み対象にしない
- これにより、本番・開発の秘密鍵や設定が誤って混在しないようにする
